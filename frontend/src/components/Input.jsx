...
function Input({ type, placeholder, setuserdata, key2, value, image }) {
  ...
  return (
    <div className="w-full relative ">
      <i
        className={`  fi fi-${image} text-xl left-2 opacity-30 absolute top-[55%] -translate-y-1/2`}
      ></i>
      ...
      {key2 == "password" && (
        <i
          ...
          className={`fi ${showpassword ? "fi-rr-eye" : "fi-rr-eye-crossed"} text-xl right-7 opacity-30 absolute top-[55%] -translate-y-1/2`}
        ></i>
      )}
    </div>
  );
